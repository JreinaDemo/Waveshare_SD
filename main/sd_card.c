...
/**************************************************************************************************/
/** @fcn        esp_err_t i2c_master_init(void)
 *  @brief      i2c master initialization
 *  @details    x
 *
 *  @return   (esp_err_t) api resp code
 */
/**************************************************************************************************/
esp_err_t i2c_master_init(void) {
	...
	//Install I2C driver
	stat = i2c_driver_install(i2c_master_port, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);
...
/**************************************************************************************************/
/** @fcn        esp_err_t waveshare_sd_card_init(void)
 *  @brief      x
 *  @details    x
 *
 ...
 *
 *	@section 	Opens
 *		Unroll if() conditionals
 *		#ifdef!
 *
 *	@section  	Assumptions\
 *		CONFIG_EXAMPLE_FORMAT_IF_MOUNT_FAILED is not defined (so format_if_mount_failed is false)
 */
/**************************************************************************************************/
...
/**************************************************************************************************/
/** @fcn        esp_err_t waveshare_sd_card_test(void)
 *  @brief      x
 *  @details    x
 *
 *  @param    [in]  name    descrip
...
esp_err_t waveshare_sd_card_test(void) {
    ...
    ret = rename(file_hello, file_foo);
    ...
    //Complete, unmount partition and disable SPI peripheral
...
/**************************************************************************************************/
/** @fcn        static esp_err_t s_example_write_file(const char *path, char *data)
 *  @brief      x
 *  @details    x
 *
 *  @param    [in]  name    descrip
...
/**************************************************************************************************/
/** @fcn        static esp_err_t s_example_read_file(const char *path)
 *  @brief      x
 *  @details    x
 *
 *  @param    [in]  (const char *) path - x
 *
 *  @return   (esp_err_t) api resp code
 *
 *	@section 	Opens
 *		Unroll if() conditionals
 *		if(f == NULL) subroutine for re-use
 */
/**************************************************************************************************/
static esp_err_t s_example_read_file(const char *path) {
